...
import { WaitSkeleton } from "@components/controls";

import { fluidCSS } from "@framework";

import { TooltipGhost } from "@framework";
import { getThemeLuminance, modelsFormat } from "@framework";
...
class PriceProjectionCard extends Component {
  render() {
    ...
    if (loading) {
      return (
        <PaperP elevation={3}>
          <WaitSkeleton loading={true} w="50px" h="50px" />
        </PaperP>
      );
    }
    return (
    ...
    if (loading) {
      return (
        <PaperP p_min={2} p_max={10} className="flex-col">
          <WaitSkeleton loading width="100%" height="50px" />
        </PaperP>
      );
    }
    return (
    ...
    if (loading) {
      return (
        <PaperP
          ...
          elevation={3}
        >
          <WaitSkeleton loading={true} w="100%" h="80px" />
        </PaperP>
      );
    }
